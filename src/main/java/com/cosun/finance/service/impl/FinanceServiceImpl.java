...
import com.cosun.finance.mapper.FinanceMapper;
import com.cosun.finance.service.IFinanceServ;
import jxl.Cell;
import jxl.CellType;
import jxl.Workbook;
...
public class FinanceServiceImpl implements IFinanceServ {
    ...
    public List<Salary> translateExcelToBean(MultipartFile file) throws Exception {
        ...
        try {
            ...
            Workbook Workbook = jxl.Workbook.getWorkbook(file.getInputStream(), ws);//它是专门读取.xls的
            if (Workbook != null) {
    ...
    public List<EmpHours> translateExcelToBeanEmpHours(MultipartFile file1, String yearMonth) throws Exception {
        ...
        try {
            WorkbookSettings ws = new WorkbookSettings();
            jxl.Sheet xlsfSheet = null;
            Workbook Workbook = jxl.Workbook.getWorkbook(file1.getInputStream(), ws);//它是专门读取.xls的
            if (Workbook != null) {
                jxl.Sheet[] sheets = Workbook.getSheets();
                xlsfSheet = sheets[0];
            }
    ...
    public void saveAllFinanceImportData(List<FinanceImportData> financeImportDataList, String yearMonth) throws Exception {
        ...
        for (FinanceImportData fid : financeImportDataList) {
            ...
            if(dept==null) {
                ...
                financeMapper.saveDeptByBean(dept);
            }
    ...
    public List<SalaryDataOutPut> computeSalaryData(String yearMonth) throws Exception {
        ...
        for (EmpHours eh : ehList) {
            ...
            if (fid != null && sa != null) {
                if (empNo.equals(fid.getEmpNo()) && name.equals(fid.getName()) && basicTotalHours <= fid.getBasicWorkHours()) {
    ...
    public List<Dept> findAllDeptAll() throws Exception {
        return financeMapper.findAllDeptAll();
    }

    public List<Employee> findAllEmployeeAll() throws Exception {
        return financeMapper.findAllEmployeeAll();
    }
    ...
    public int queryEmployeeSalaryByConditionCount(Employee employee) throws Exception {
        return financeMapper.queryEmployeeSalaryByConditionCount(employee);
    }
